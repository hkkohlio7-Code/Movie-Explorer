  ...
  const navigate = useNavigate()
    ...
    useEffect(()=>{
        ...
        .then(data => {
            ...
            return setPopular(data.results);
        });
    }, [])
  return (
    <div className='w-full flex justify-center'>
        <div className='lg:w-[80%] w-[95%] flex flex-col gap-2'>
          <h1 className='lg:text-2xl text-xl'>What's Popular</h1>
          <div className='flex overflow-x-auto gap-5 lg:py-6 py-3 lg:px-3 cast-scrollbar rounded-2xl lg:border border-gray-300 lg:shadow-2xl'>
            {popular.map((pop)=>{
              return <div key={pop.id} className='lg:h-90 h-75 w-40 border border-gray-300 shadow-2xl shrink-0 overflow-hidden rounded-2xl hover:scale-105 transition-transform duration-300' onClick={()=>navigate(`/movie?query=${pop.id}`)}>
                <img src={`${URL}${pop.poster_path}`} className='lg:h-56 h-48 w-full object-center' loading="lazy"/>
                <div className='px-2 py-1' >
                    <h1 className='lg:text-lg text-medium'>
                        {pop.title}
                    </h1>
                    <p className='lg:text-sm text-xs text-gray-800'>{new Date(pop.release_date).toLocaleDateString("en-GB", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                        })}
